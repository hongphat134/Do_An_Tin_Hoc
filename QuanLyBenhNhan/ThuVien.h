...
//( Ngày khám của bệnh nhân, Ngày sinh của bệnh nhân )
struct Ngay{
	int ngay, thang, nam;
...
// Toa Thuốc của ngày khám nào đó của bệnh nhân ?
struct ToaThuoc{
	NODE_Thuoc* thuoc;
	string cachdung;
	int soluong;
...
//Các dịch vụ mà bệnh nhân dùng trong ngày khám nào đó?
struct DichVuVLYT{
	NODE_VLYT* vlyt;
	int gia;
};

//thành phần của số khám bệnh
struct KhamBenh{
	Ngay NgayKham;
	string trieuchung;
	string ChanDoan;
...
//thành phần của Danh Sách Bệnh nhân
struct BenhNhan{
	int ma;
	string ten;
	Ngay ngaysinh;
...
//tìm kiếm theo tiêu chí mã,tên, ngày sinh
...
//Thống kê số lượng bệnh nhân + doanh thu theo ngày , tháng , năm , từ ngày -> đến ngày
